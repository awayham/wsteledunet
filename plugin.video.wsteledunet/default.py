...
def list_categories():
    items = [{
                 ...
                 'label': 'Free',
                 'path': plugin.url_for('list_free_channels'),
                 'thumbnail': _art('art', 'all.png')
...
def list_free_channels():
    items = [{
                 ...
                 'thumbnail': channel.thumbnail,
                 'is_playable': True,
             } for channel in api.get_free_channels()]
...
#    return plugin.set_resolved_url(playback_url)
    plugin.set_resolved_url(playback_url)
    api.update_idu()
